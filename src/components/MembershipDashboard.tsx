      ...
      <div className="space-y-6 p-6 pb-24 text-center">
        <div className="rounded-2xl border border-red-200 bg-red-50 p-12 shadow-sm">
          <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4">
            <svg className="w-8 h-8 text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M6 18L18 6M6 6l12 12" /></svg>
          </div>
          <h3 className="text-2xl font-black text-red-700">Membership Expired</h3>
          <p className="mt-2 text-sm text-red-600/80 max-w-xs mx-auto">Your access has expired. Please visit the front desk or use the options below.</p>
        </div>
  ...
  return (
    <section className="rounded-2xl border border-flexNavy/15 bg-flexWhite/70 p-12 shadow-sm text-center">
       <div className="w-16 h-16 bg-flexBlue/10 rounded-full flex items-center justify-center mx-auto mb-4">
         <svg className="w-8 h-8 text-flexBlue" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M12 4v16m8-8H4" /></svg>
       </div>
       <h3 className="text-2xl font-black text-[#071731]">No Active Membership</h3>
       <p className="mt-2 text-sm text-slate-600 max-w-xs mx-auto">You need an active plan to access gym features and track your progress.</p>
       <button onClick={() => window.location.href='/subscription-tier'} className="mt-6 px-8 py-3 bg-flexBlue text-white rounded-xl font-bold shadow-lg shadow-flexBlue/20 hover:-translate-y-0.5 transition-all">Get Started Now</button>
    </section>
  );
}
...
function MembershipFAB() {
  ...
  return (
    ...
    >
      {isCheckedIn ? (
        <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path d="M17 16l4-4m0 0l-4-4m4 4H7" /></svg>
      ) : (
